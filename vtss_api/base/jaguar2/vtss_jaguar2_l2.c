...
{
    u32 upsid = vtss_state->l2.vstax_info.upsid[0];
...
{
    u32 mach, macl, cfg2, addr, upsid = 0, aged = 0, fwd_kill = 0;
    ...
    } else if (entry->vstax2.enable) {
        /* Use (UPSID, UPSPN) */
        addr = entry->vstax2.upspn;
        upsid = entry->vstax2.upsid;
    } else {
...
{
    u32                cfg2, mach, macl, addr, type, glag_no;
    vtss_port_no_t     port_no;
    vtss_pgid_entry_t  *pgid_entry;
    ...
    entry->copy_to_cpu = VTSS_BOOL(cfg2 & VTSS_M_LRN_COMMON_MAC_ACCESS_CFG_2_MAC_ENTRY_CPU_COPY);
    ...
    case MAC_ENTRY_ADDR_TYPE_UPSID_CPU_OR_INT:
        my_upsid = vtss_state->l2.vstax_info.upsid[0];
        upsid = ((addr >> 5) & 0x1f);
        entry->vstax2.enable = 1;
        entry->vstax2.remote_entry = (upsid == my_upsid || upsid == (my_upsid + 1) ? 0 : 1);
        entry->vstax2.upsid = upsid;
        if (type == MAC_ENTRY_ADDR_TYPE_UPSID_PN) {
            entry->vstax2.upspn = (addr & 0x1f);
            *pgid = vtss_jr2_vtss_pgid(vtss_state, entry->vstax2.upspn + (upsid == (my_upsid + 1) ? 32 : 0));
        } else {
                                     ...
                                     BOOL             age)
{
    ...
    JR2_WR(VTSS_LRN_COMMON_MAC_ACCESS_CFG_0, VTSS_F_LRN_COMMON_MAC_ACCESS_CFG_0_MAC_ENTRY_FID(vid));
                                       ...
                                       const vtss_port_no_t port_no)
{
    ...
    JR2_WRM_PMASK(VTSS_ANA_L2_COMMON_LRN_SECUR_CFG,        (mode->discard   ? pmask : pmask_zero), pmask);
    JR2_WRM_PMASK(VTSS_ANA_L2_COMMON_LRN_SECUR_LOCKED_CFG, (mode->discard   ? pmask : pmask_zero), pmask);
    JR2_WRM_PMASK(VTSS_ANA_L2_COMMON_AUTO_LRN_CFG,         (mode->automatic ? pmask : pmask_zero), pmask);
    JR2_WRM_PMASK(VTSS_ANA_L2_COMMON_LRN_COPY_CFG,         (mode->cpu       ? pmask : pmask_zero), pmask);
...
static vtss_rc jr2_vlan_mask_apply(vtss_state_t *vtss_state, vtss_vid_t vid, u64 pmask)
{
    ...
    JR2_WRM(VTSS_ANA_L3_VLAN_VLAN_CFG(vid), 
            VTSS_F_ANA_L3_VLAN_VLAN_CFG_VLAN_FID(vid) |
            VTSS_F_ANA_L3_VLAN_VLAN_CFG_VLAN_LRN_DIS(!vlan_entry->conf.learning) |
            VTSS_F_ANA_L3_VLAN_VLAN_CFG_VLAN_PRIVATE_ENA(vlan_entry->isolated) |
            VTSS_F_ANA_L3_VLAN_VLAN_CFG_VLAN_MIRROR_ENA(vlan_entry->conf.mirror),
                                    ...
                                    BOOL         member[VTSS_PORT_ARRAY_SIZE]) 
{
    ...
    VTSS_RC(jr2_vlan_mask_apply(vtss_state, vid, pmask));
    return VTSS_RC_OK;
} // jr2_vlan_mask_update
                                      ...
                                      vtss_vlan_port_conf_t *conf)
{
    ...
    JR2_WR(VTSS_ANA_CL_PORT_VLAN_TPID_CTRL(port),
           VTSS_F_ANA_CL_PORT_VLAN_TPID_CTRL_BASIC_TPID_AWARE_DIS(0x7fff - (aware ? (1 << tpid) : 0)) |
           VTSS_F_ANA_CL_PORT_VLAN_TPID_CTRL_RT_TAG_CTRL(3));
                                         ...
                                         vtss_port_no_t port_no, vtss_vlan_port_conf_t *conf)
{
    ...
    return vtss_jr2_vlan_port_conf_apply(vtss_state, port, conf);
} // jr2_vlan_port_conf_update
...
static vtss_rc jr2_mirror_conf_set(vtss_state_t *vtss_state)
{
    ...
    vtss_port_no_t     port_no;
    u32                probe, port = 0, dir, vlan_mode, tx_port;
    ...
    for (probe = JR2_MIRROR_PROBE_RX; probe <= JR2_MIRROR_PROBE_VLAN; probe++) {
        ...
        if (probe == JR2_MIRROR_PROBE_RX) {
            /* Ingress probe */
            pmask_probe = (pmask ? vtss_jr2_port_mask(vtss_state, vtss_state->l2.mirror_ingress) : pmask);
        } else if (probe == JR2_MIRROR_PROBE_TX) {
            /* Egress probe */
            dir = 1;
            pmask_probe = (pmask ? vtss_jr2_port_mask(vtss_state, vtss_state->l2.mirror_egress) : pmask);
            ...
            pmask_probe = (pmask ? vtss_jr2_port_mask(vtss_state, member) : pmask);
        }
...
static vtss_rc jr2_sflow_sampling_rate_convert(struct vtss_state_s *const state, const BOOL power2, const u32 rate_in, u32 *const rate_out)
{
    ...
    return VTSS_RC_OK;
} // jr2_sflow_sampling_rate_convert
...
#define JR2_SFLOW_ENABLED(_conf_) ((_conf_)->sampling_rate > 0 && (_conf_)->type != VTSS_SFLOW_TYPE_NONE)
    ...
    *cur_conf = *new_conf;
    ...
    JR2_WRM(VTSS_ANA_AC_SFLOW_SFLOW_CTRL(port_no),
                              ...
                              const vtss_debug_info_t   *const info)
{
    ...
    for (vid = VTSS_VID_NULL; vid < VTSS_VIDS; vid++) {
        ...
        JR2_RDX_PMASK(VTSS_ANA_L3_VLAN_VLAN_MASK_CFG, vid, &pmask);
        JR2_RD(VTSS_ANA_L3_VLAN_VLAN_CFG(vid), &value);

        if (header)
            jr2_debug_pmask_header(vtss_state, pr, "VID   Lrn  Mir  Prv");
        header = 0;

        sprintf(buf, "%-6u%-5u%-5u%-5u", vid,
                value & VTSS_M_ANA_L3_VLAN_VLAN_CFG_VLAN_LRN_DIS     ? 0 : 1,
                value & VTSS_M_ANA_L3_VLAN_VLAN_CFG_VLAN_MIRROR_ENA  ? 1 : 0,
                value & VTSS_M_ANA_L3_VLAN_VLAN_CFG_VLAN_PRIVATE_ENA ? 1 : 0);
                                   ...
                                   const vtss_debug_info_t   *const info)
{
    ...
    /* Flood masks */
    VTSS_RC(jr2_debug_pgid_table(vtss_state, pr, info, PGID_UC_FLOOD, PGID_IPV6_MC_CTRL + 1));
...
static vtss_rc jr2_debug_glag(vtss_state_t *vtss_state,
                              const vtss_debug_printf_t pr,
                              const vtss_debug_info_t   *const info)
{
#if defined(VTSS_FEATURE_AGGR_GLAG)
    u32 ac, id, port, pgid, value, gl_memb[VTSS_GLAGS];
    u64 pmask;

    pr("%-10s %-10s %-10s %-10s\n", "Port", "Lport","is-Glag","Glag_no");
    for (port = 0; port < VTSS_CHIP_PORTS; port++) {
        JR2_RD(VTSS_ANA_CL_PORT_PORT_ID_CFG(port), &value);
        pr("%-10u %-10u %-10d %-10u\n", port, 
           VTSS_X_ANA_CL_PORT_PORT_ID_CFG_LPORT_NUM(value), 
           VTSS_X_ANA_CL_PORT_PORT_ID_CFG_PORT_IS_GLAG_ENA(value),
           VTSS_X_ANA_CL_PORT_PORT_ID_CFG_GLAG_NUM(value));
    }
    pr("\n");
    ...
    for (id = 0; id < VTSS_GLAGS; id++) {
        JR2_RD(VTSS_ANA_AC_GLAG_MBR_CNT_CFG(id), &value); 
        pr("%-8u %-8u\n", id, value+1);
        gl_memb[id] = value;
    }
    pr("\n");

    vtss_debug_print_header(pr, "Aggregation settings");
    JR2_RD(VTSS_ANA_CL_COMMON_AGGR_CFG, &value);
    pr("ANA_CL:COMMON:AGGR_CFG=0x%08x\n", value);
    pr("\n");

    vtss_debug_print_header(pr, "GLAG Source Masks");
    vtss_jr2_debug_print_port_header(vtss_state, pr, "Glag  ");
    for (id = ANA_AC_SRC_GLAG_OFFSET; id < VTSS_GLAGS + ANA_AC_SRC_GLAG_OFFSET; id++) {
        JR2_RDX_PMASK(VTSS_ANA_AC_SRC_SRC_CFG, id, &pmask);
        pr("%-4u  ", id-ANA_AC_SRC_GLAG_OFFSET);
        vtss_jr2_debug_print_pmask(pr, pmask);
    }
    pr("\n");
    
    vtss_debug_print_header(pr, "Aggregation Masks");
    vtss_jr2_debug_print_port_header(vtss_state, pr, "AC    ");
    for (ac = 0; ac < 16; ac++) {
        JR2_RDX_PMASK(VTSS_ANA_AC_AGGR_AGGR_CFG, ac, &pmask);
        pr("%-4u  ", ac);
        vtss_jr2_debug_print_pmask(pr, pmask);
    ...
    for (pgid = PGID_GLAG_START; pgid < (PGID_GLAG_START+VTSS_GLAG_PORTS*VTSS_GLAGS); pgid++) {
        ...
        pr("%-8u %-8u %-8d %-8d %-8u %-8u %-8u", pgid, id,
           ...
           VTSS_X_ANA_AC_PGID_PGID_MISC_CFG_PGID_CPU_COPY_ENA(value),
           VTSS_X_ANA_AC_PGID_PGID_MISC_CFG_PGID_CPU_QU(value),
           (u32)(pmask >> 6 & 0x1F), (u32)(pmask & 0x1F));
...
static vtss_rc jr2_debug_lrn_table(vtss_state_t *vtss_state,
                                  const vtss_debug_printf_t pr,
                                  const vtss_debug_info_t   *const info)
{
    ...
    vtss_jr2_debug_sticky(vtss_state, pr, VTSS_LRN_COMMON_EVENT_STICKY, "EVENT_STICKY");
                             ...
                             const vtss_debug_info_t   *const info)
{
    ...
    jr2_debug_pmask_header(vtss_state, pr, NULL);
    JR2_RDX_PMASK(VTSS_ANA_L3_MSTP_MSTP_LRN_CFG, 0, &pmask);
    jr2_debug_pmask(pr, "MSTP_LRN_CFG_0", pmask);
...
#if defined(VTSS_FEATURE_AGGR_GLAG)
    VTSS_RC(vtss_debug_print_group(VTSS_DEBUG_GROUP_GLAG,      jr2_debug_glag,      vtss_state, pr, info));
#endif
    VTSS_RC(vtss_debug_print_group(VTSS_DEBUG_GROUP_LRN,       jr2_debug_lrn_table, vtss_state, pr, info));
...
static vtss_rc jr2_l2_init(vtss_state_t *vtss_state)
{
    ...
    for (port = 0; port < VTSS_CHIP_PORTS; port++) {
        VTSS_RC(vtss_jr2_vlan_port_conf_apply(vtss_state, port, &state->vlan_port_conf[VTSS_PORT_NO_START]));
    }

    /* Enable forwarding and learning */
    JR2_WRX_PMASK(VTSS_ANA_L3_MSTP_MSTP_FWD_CFG, 0, port_mask_all);
