...
static const tcam_props_t tcam_info[] = {
    ...
    [VTSS_TCAM_IS2] = {
        ...
        .action_words = BITS_TO_DWORD(103 - 2*VCAP_PORT_OFFSET),
        .action_width = (103 - 2*VCAP_PORT_OFFSET),
        .action_type_width = 1,
        .action_type_0_width = (51 - VCAP_PORT_OFFSET),
...
#undef MASK
            ...
            // Action
            mask = vtss_srvl_port_mask(vtss_state, action->mll.b_portlist.physical) | (action->mll.b_portlist.cpu ? (1<<11) : 0);
    ...
    case VTSS_IS0_TYPE_MLBS:
        {
            ...
            // Action
            mask = vtss_srvl_port_mask(vtss_state, action->mlbs.b_portlist.physical) | (action->mlbs.b_portlist.cpu ? (1<<11) : 0);
                                  ...
                                  vtss_vcap_idx_t *idx, vtss_vcap_data_t *vcap_data, u32 counter)
{
    ...
    case VTSS_IS1_TYPE_ETYPE:
        ...
        if (vtss_state->misc.chip_id.revision != 0 &&
            ...
            (key->isdx.value[0] & key->isdx.mask[0] & 0x02) == 0) {
            /* Match OAM frames on revision B using ISDX bit 9 */
            info.etype.mask[1] = 0;
    ...
    if (idx->key_size == VTSS_VCAP_KEY_SIZE_QUARTER) {
        ...
        if (type == IS1_TYPE_NORMAL) {
            BOOL dmac_dip = FALSE;
            vtss_port_no_t first_port = vtss_cmn_first_port_no_get(vtss_state, key->port_list, 0);
                                  ...
                                  vtss_vcap_idx_t *idx, vtss_vcap_data_t *vcap_data, u32 counter)
{
    ...
    srvl_vcap_key_set(data, ES0_FKO_VID, ES0_FKL_VID, 
                      ...
                      key->vid_any ? 0 : VTSS_BITMASK(ES0_FKL_VID));
           ...
           Step 2: Update resources */
            ...
            for (cur = obj->used; cur != NULL; cur = cur->next) {
                ...
                if ((cur->data.u.is1.lookup != lookup) ||
                    (vtss_cmn_first_port_no_get(vtss_state, copy->key.port_list, 0) != port_no)) {
