...
typedef struct vtss_ewis_rdil_cons_act_s {
    ...
    BOOL rdil_on_lopc;  /**< TRUE = enable for RDI-L backreporting on LOPC, not supported in 8492 */
    BOOL rdil_on_ais_l; /**< TRUE = enable for RDI-L backreporting on AIS_L, not supported in 8492 */
...
typedef struct vtss_ewis_defects_s {
    ...
    BOOL dlcd_p;        /**< Path loss of code-group delineation, not supported in 8492 */
...
typedef enum {
    ...
    EWIS_PRBS31_MODE_MAX
} vtss_ewis_prbs31_err_inj_t;

/** \brief eWIS static configuration data, 
 *  \note This is specific to 8487/8488-15 and should not be used for Daytona.
 */
...
#if defined (VTSS_ARCH_DAYTONA) && !defined (VTSS_CHIP_10G_PHY)
#define VTSS_EWIS_LOS_EV            0x0000000100000000  /**< Loss of Signal */
#define VTSS_EWIS_LOF_EV            0x0000000200000000  /**< Loss of Frame */
#define VTSS_EWIS_RDIL_EV           0x0000000400000000  /**< Line Remote Defect Indication */
#define VTSS_EWIS_AISL_EV           0x0000000800000000  /**< Line Alarm Indication Signal */
#define VTSS_EWIS_AISP_EV           0x0000001000000000  /**< Path Alarm Indication Signal */
#define VTSS_EWIS_PLMP_EV           0x0000002000000000  /**< Path Label Mismatch */
#define VTSS_EWIS_UNEQP_EV          0x0000004000000000  /**< Path Unequipped */
#endif /* VTSS_ARCH_DAYTONA && !VTSS_CHIP_10G_PHY */

#if defined(VTSS_ARCH_DAYTONA)
#define VTSS_EWIS_OOF_EV            0x0000008000000000  /**< Out of Frame */
#define VTSS_EWIS_S1_EV             0x0000010000000000  /**< Synchronous Message */
#define VTSS_EWIS_LOP_EV            0x0000020000000000  /**< Loss of Pointer */
#define VTSS_EWIS_RDIP_EV           0x0000040000000000  /**< Path Remote Defect Indication */
#define VTSS_EWIS_FE_AIS_P_EV       0x0000080000000000  /**< Far End Path Alarm Indication Signal */
#define VTSS_EWIS_FEPLMP_EV         0x0000100000000000  /**< Far End Path Payload Mismatch */
#define VTSS_EWIS_FE_UNEQP_EV       0x0000200000000000  /**< Far End Path Unequipped */
 ...
 * In Daytona this is useful in setting the WIS block to operate in multiple modes.
 *
 * \param inst [IN]      Target instance reference.
 * \param port_no [IN]   Port number.
 * \param mode [IN]      Set WIS mode (Disable, WIS, STS192, STM64).
 *                       sts192 (full Sonet/SDH termination is only supported in Daytona)
 *
 * \return Return code.
 **/
...
#if 0
/**
 * \brief Initialize the eWIS block, ewis_conf in vtss_state is set to defaults.
 * \note  The default mode is set to LAN.
 *
 * In 8487/8488-15 The user should call vtss_phy_10g_mode_set with oper_mode set
 * to VTSS_PHY_WAN_MODE and with appropriate wrefclk configuration to set Phy in WAN mode.
 *
 * In Daytona the user should call the vtss_ewis_mode_set with appropriate wis_mode
 * to be able to change between the operating modes.
 *
 * This function should be called once for each port/channel in the PHY, and this 
 * should be called first before calling other ewis functions.
 *
 * The default setup is
 * 1) The defect detection is disabled.
 * 2) WIS events configuration is disabled.
 * 3) J0 and J1 is set to operate in 16 Byte mode.
 * 4) Consequent actions are disabled.
 * 5) force mode is disabled.
 *
 * \param inst [IN]      Target instance reference.
 * \param port_no [IN]   Port number.
 *
 * \return Return code.
 **/
vtss_rc vtss_ewis_static_init(const vtss_inst_t      inst,
                              const vtss_port_no_t   port_no);
#endif /* 0 */
